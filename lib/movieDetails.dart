...
import './models/movieCreditsModel-dart.dart';
import './models/keysDb.dart' as keysDb;
...
class _MovieDetails extends State<MovieDetail>{
  ...
  void initState() {
    ...
    super.initState();
    movieDetailUrl = "${keysDb.baseUrl}${widget.movie.id}?api_key=${keysDb.apiKey}";
    movieCreditsUrl = "${keysDb.baseUrl}${widget.movie.id}/credits?api_key=${keysDb.apiKey}";
    _fetchMovieDetails();
    _fetchMovieCredits();
  }
  ...
  void _fetchMovieCredits() async {
    ...
    movieCredits = MovieCreditsModel.fromJson(decodedJson);
                        ...
                        children: <Widget>[
                          CircleAvatar(
                              ...
                              backgroundImage: c.imagePath != null
                                  ? NetworkImage("${keysDb.baseImageUrl}w154${c.imagePath}")
                                  : AssetImage('assets/steven.png')
                          ),
                          Padding(
  ...
  Widget build(BuildContext context) {
      ...
      child: Center(
        child: Card(
          ...
          child: Hero(
              ...
              child: Image.network("${keysDb.baseImageUrl}w342${widget.movie.posterPath}",
              fit: BoxFit.cover
              )
          )
        ),
    ...
    final genresList = Container(
      height: 34.0,
      child: Padding(
        padding: const EdgeInsets.only(left: 10.0, top: 6.0, bottom: 4.0),
        child: ListView(
          scrollDirection: Axis.horizontal,
          children: movieDetails == null
              ? []
              : movieDetails.genres.map((g) => Padding(
